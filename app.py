...
if not os.path.exists(LOG_FILE):
    with open(LOG_FILE, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Ping Count", "Last Ping", "Status Code"])
...
def ping_website():
    ...
    try:
        ...
        ping_count += 1
        last_ping = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
        with open(LOG_FILE, "a", newline="") as f:
            csv.writer(f).writerow([ping_count, last_ping, response.status_code])
    except Exception:
        pass
...
def start_pinger():
    ...
    if not pinger_running:
        ...
        ping_website()  # Immediate first ping
